			...
			<div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
				<div className="mb-8">
					...
					<p className="mt-2 text-sm text-gray-600 sm:text-base">
						Calcula el crecimiento de tus ahorros con nuestra herramienta de
						simulación
					</p>
				</div>

				<div className="mb-8 rounded-lg bg-white p-6 shadow-lg">
					<form onSubmit={handleSubmit} className="space-y-6 text-main-bg">
						{/* Initial Amount */}
						<div>
							<label
							...
							<input
								...
								placeholder="$0"
								className={`w-full rounded-lg border px-4 py-3 text-lg transition-all duration-200 focus:outline-none focus:ring-2 ${errors.initialAmount
						...
						<div>
							...
							<input
								...
								placeholder="$0"
								className={`w-full rounded-lg border px-4 py-3 text-lg transition-all duration-200 focus:outline-none focus:ring-2 ${errors.monthlyContribution
									? 'border-red-300 focus:border-red-500'
									: 'border-gray-300 focus:border-cyan-400'
									}`}
							/>
						...
						<div>
							...
							<input
								...
								onChange={(e) => {
									...
									setMonths(newValue)
									validateMonths(newValue, newValue, setErrors)
								}}
								placeholder="0"
								min="5"
								className={`w-full rounded-lg border px-4 py-3 text-lg transition-all duration-200 focus:outline-none focus:ring-2 ${errors.months
									? 'border-red-300 focus:border-red-500'
									: 'border-gray-300 focus:border-cyan-400'
									}`}
							/>
						...
						<div className="flex gap-4">
							<button
								type="submit"
								disabled={!isFormValid(initialAmount, monthlyContribution, months)}
								className="flex-1 rounded-lg bg-cyan-500 px-6 py-3 font-semibold text-white transition-all hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 disabled:bg-gray-300 disabled:cursor-not-allowed disabled:hover:opacity-50"
							>
								Calcular
							</button>
							<button
								type="button"
								onClick={handleReset}
								className="rounded-lg border-2 border-cyan-500 px-6 py-3 font-semibold text-cyan-500 transition-all hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
							>
								Limpiar
							</button>
						</div>
							...
							<div className="flex items-start">
								...
								<div className="ml-3">
									<p className="text-sm text-slate-700">
										...
										El cálculo se basa en una tasa de interés mensual estimada del
										0.5% (6% anual efectivo). Los rendimientos reales pueden variar
										según las condiciones del mercado y el tipo de cuenta seleccionada.
									</p>
								</div>
							</div>
						</div>
				...
				{!result && (
					<div className="mt-6 rounded-lg border border-cyan-500/25  p-4 bg-secondary/20">
						<div className="flex items-start">
							<ExclamationCircleIcon className="mt-0.5 h-5 w-5 shrink-0 text-cyan-500" />
							<div className="ml-3">
								<p className="text-sm text-slate-700">
									...
									mensuales deben estar entre $10,000 y $5,000,000 COP. El plazo debe
									estar entre 5 y 120 meses (máximo 10 años). El simulador utiliza una
									tasa de interés mensual del 0.5% para fines de demostración.
								</p>
							</div>
						</div>
					</div>
